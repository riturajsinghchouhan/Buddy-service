...
export default function RestaurantOTP() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0a0a] flex flex-col relative overflow-hidden font-['Poppins']">
      ...
      <div className="absolute top-[-100px] right-[-100px] w-[500px] h-[500px] bg-[#23361A]/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="relative z-20 px-6 py-8 flex items-center">
        <motion.button
          ...
          className="p-3 bg-white dark:bg-[#1a1a1a] shadow-xl shadow-[#23361A]/10 rounded-2xl text-[#23361A] border border-[#23361A]/5 outline-none"
        >
        ...
        >
          ...
          <div className="text-center mb-12">
            <motion.div
              ...
              className="w-20 h-20 bg-[#23361A] rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-[#23361A]/30 relative"
            >
              <ShieldCheck className="text-white w-10 h-10" />
            </motion.div>
            
            <h1 className="text-4xl font-black text-[#23361A] font-['Outfit'] tracking-tight mb-3">
              Verify Account
            </h1>
            <p className="text-gray-500 dark:text-gray-400 font-medium">
              We've sent a 4-digit code to <br />
              <span className="text-[#23361A] font-bold">{contactInfo}</span>
            </p>
          </div>
          ...
          <div className="bg-white/80 dark:bg-[#1a1a1a]/80 backdrop-blur-2xl rounded-[3rem] p-10 shadow-[0_40px_80px_-20px_rgba(126,56,102,0.2)] border border-white/20 dark:border-gray-800">
              ...
              {otp.map((digit, index) => (
                <div key={index} className="relative group">
                  <input
                    ...
                    className={`w-full aspect-square bg-gray-50 dark:bg-gray-900/50 text-center text-3xl font-black text-[#23361A] border-2 border-transparent rounded-2xl outline-none transition-all ${
                      focusedIndex === index 
                        ? "border-[#23361A] bg-white scale-105 shadow-[0_10px_30px_rgba(126,56,102,0.1)]" 
                        : "group-hover:border-gray-200 dark:group-hover:border-gray-700"
                    }`}
                  />
                  <div className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-8 h-1 rounded-full transition-all duration-300 ${focusedIndex === index ? "bg-[#23361A] opacity-100" : "bg-gray-200 opacity-0"}`} />
                </div>
              ))}
            </div>
            ...
            <button
              ...
              className="w-full py-4.5 bg-[#23361A] hover:bg-[#1a2614] disabled:bg-gray-200 dark:disabled:bg-gray-800 disabled:text-gray-400 text-white rounded-2xl font-bold text-lg shadow-xl shadow-[#23361A]/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2 mb-8"
            >
            ...
            <div className="text-center">
              {resendTimer > 0 ? (
                <p className="text-sm text-gray-400 font-medium flex items-center justify-center gap-2 tracking-wide uppercase text-[10px] font-black">
                  <Timer className="w-3.5 h-3.5 text-[#23361A]" />
                  Resend code in <span className="text-[#23361A] font-bold">{resendTimer}s</span>
                </p>
              ) : (
                <button
                  onClick={handleResend}
                  className="text-xs text-[#23361A] font-black uppercase tracking-widest hover:underline underline-offset-4 flex items-center justify-center gap-2 mx-auto"
                >

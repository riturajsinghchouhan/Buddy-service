        ...
        {showRestaurantsSkeleton ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5 xl:gap-6 pt-2 sm:pt-3 lg:pt-4">
            {restaurants.map((restaurant, index) => {
              ...
              const chainDistanceBadge = lastCartRestaurant
                ? getChainDistanceBadge(lastCartRestaurant, {
              ...
              return (
                <ScrollReveal key={restaurant.id} delay={index * 0.05}>
                  ...
                  >
                    <Card className="overflow-hidden cursor-pointer border border-gray-200 dark:border-gray-800 group bg-white dark:bg-[#1a1a1a] hover:shadow-lg dark:hover:shadow-xl dark:hover:shadow-gray-900/50 pb-1 sm:pb-2 lg:pb-3 flex flex-col h-full w-full transition-all duration-300 relative">
                          ...
                          <div className="flex-1 flex flex-col justify-between gap-2">
                            <div className="flex-shrink-0">
                              <div className="flex items-start justify-between gap-2 mb-2">
                                ...
                                >
                                  <Heart
                                    className={`h-4 w-4 sm:h-5 sm:w-5 ${favorite ? "fill-red-500 dark:fill-red-400" : ""}`}
                                  />
                                </Button>
                              </div>
                            </div>
                      ...
                      {chainDistanceBadge && (
                        <div
                          className={`absolute bottom-2 right-2 z-20 max-w-[min(100%,14rem)] rounded-lg px-2 py-1 text-[10px] sm:text-[11px] font-semibold leading-tight shadow-md border ${
                            chainDistanceBadge.withinRadius
                              ? "bg-emerald-600/95 text-white border-emerald-500/80"
                              : "bg-rose-600/95 text-white border-rose-500/80"
                          }`}
                        >
                          {chainDistanceBadge.label}
                        </div>
                      )}
                    </Card>
                  </Link>
